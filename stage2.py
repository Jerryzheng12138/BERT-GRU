...
class Config(object):
    def __init__(self, dataset):
        ...
        self.dataset_path = f"{dataset}/data/2nd/test/test-antigram-negative.txt"      #/data/2nd/test/test-anti_mammalian_cells
        self.class_list = [x.strip() for x in open(dataset + '/data/class2.txt').readlines()]
...
weight_files = [f'./data/saved_dict/2nd/antigram-negative/bertGRU antigram-negative.ckpt_fold{i}' for i in range(1, 11)] #anti_mammalian_cells/bertGRU anti_mammalian_cells/  for i in range(1, 11)

threshold_df = pd.read_csv('./data/data/2nd_threashold/antigram-negative/thresholds.csv')  #data\data\2nd_threashold\anti-mammalian_cells\thresholds.csv
threshold_df['weight_file'] = threshold_df['weight_file'].apply(lambda x: os.path.basename(x))  
threshold_dict = dict(zip(threshold_df['weight_file'], threshold_df['threshold']))
...
for weight_file in weight_files:
    ...
    if weight_file_name not in threshold_dict:
        print(f"Warning: No threshold found for {weight_file_name}. Using default value (0.5).")
        threshold = 0.5
    else:
        threshold = threshold_dict[weight_file_name]
